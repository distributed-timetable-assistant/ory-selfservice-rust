...
use crate::core::domain::flow::{SettingsUiNode, UiNodeResponse};
...
pub struct SettingsFlowHandler<I: IdentityService, R: UrlRewriter> {
    identity_service: Arc<I>,
    url_rewriter: Arc<R>,
}

impl<I: IdentityService, R: UrlRewriter> SettingsFlowHandler<I, R> {
...
#[async_trait]
impl<I: IdentityService, R: UrlRewriter> FetchUiFlowHandler<SettingsUiNode> for SettingsFlowHandler<I, R> {
    async fn fetch_ui(
        &self,
        headers: &HeaderMap,
        params: &HashMap<String, String>,
    ) -> FlowResult<UiNodeResponse<SettingsUiNode>> {
        let identity_service = self.identity_service.clone();
        let url_rewriter = self.url_rewriter.clone();
        let mut resp =
            handle_identity_fetch_flow::<SettingsUiNode>(headers, params, identity_service, url_rewriter)
                .await?;
...
#[async_trait]
impl<I: IdentityService, R: UrlRewriter> ProxySubmitUiFlowHandler<SettingsUiNode> for SettingsFlowHandler<I, R> {
    async fn proxy_submit_ui(
        ...
        let identity_service = self.identity_service.clone();
        let url_rewriter = self.url_rewriter.clone();
        let mut resp = handle_identity_submit_flow::<SettingsUiNode>(
            headers,
            params,
            body,
            identity_service,
