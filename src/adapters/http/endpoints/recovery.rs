...
use crate::ui::pages::RecoveryPage;
...
pub async fn fetch_ui(
    ...
    let flow_handler = &state.flow_handlers.recovery;
    ...
    Ok((
        ...
        resp.headers,
        resp.ui_node
            .map(|ui_node| leptos::view! { <RecoveryPage flow=ui_node /> }.to_html())
            .unwrap_or_default(),
    )
        .into_response())
}
...
pub async fn proxy_submit_ui(
    ...
    let flow_handler = &state.flow_handlers.recovery;
    let mut resp = flow_handler
        .proxy_submit_ui(&headers, &params, body)
        .await?;
    if resp.ui_node.is_some() {
        resp.headers.insert(
            CONTENT_TYPE,
